...
namespace Masquerade.Api
{
    public partial class MasqueradeApi
    {
        ...
        public ModAccessory GetModAccessoryByType(int weaponTypeId)
        {
            if (LoadedMods == null) throw new NullReferenceException(ExceptionMessages.MODS_LIST_NOT_INIT);
            if (weaponTypeId < Common.WEAPONTYPE_START_ID) throw new ArgumentOutOfRangeException(nameof(weaponTypeId));

            return AccessoryFactory.GetEquipment(weaponTypeId);
        }
        ...
        public CharacterContainer GetEquipmentOwner(ModEquipment modEquipment)
        {
            ...
            {
                LoggerHelper.Logger.Warning($"ModEquipment {modEquipment.FullName} has no instance id attached!");
                return null;
            }
            var character = _characters.SingleOrDefault(x => x.HasEquipment(modEquipment.ContentId));
            if (character == null)
            {
                LoggerHelper.Logger.Warning($"No character owns ModEquipment {modEquipment.FullName}");
                return null;
            }
